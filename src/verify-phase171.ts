/** Phase 171 Verification - External Media Transfer Credential-Acknowledged Manual Reinvoke Execution Receipt Closeout Record Plan Preflight */
...
async function executedSourceManualReinvoke() {
  const truth = await currentAction();
...
async function executionReceiptFixture() {
  const { truth, execution } = await executedSourceManualReinvoke();
  const receiptResult =
    await createExternalChannelMediaTransferManualRetryReplaySourceRevalidationCredentialAcknowledgedManualReinvokeExecutionReceipt({
      candidate: truth.candidate,
...
async function main(): Promise<void> {
  console.log("THE COLONY - Phase 171 Verification (External Media Transfer Credential-Acknowledged Manual Reinvoke Execution Receipt Closeout Record Plan Preflight)\n");
  ...
  console.log("\nPhase 171: external media transfer credential-acknowledged manual reinvoke execution receipt closeout record plan preflight is GREEN.");
}
